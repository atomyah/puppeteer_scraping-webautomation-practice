//修正必要だった。///---修正---///const aTags = await page.$$('.bkWMgd .g h3 a');
...
(async () => {
  ...
  // リンク一覧
  ///---修正---///const aTags = await page.$$('.bkWMgd .g h3 a');
  const aTags = await page.$$('.bkWMgd .g .rc .r a');
console.log('--------------------------aTagsは、' + aTags);

  for (aTag of aTags) {
    console.log('for文の中--------------------------aTagは、' + aTag);
